...
so_cal <- ejscreen %>%
  dplyr::filter(CNTY_NAME %in% c("Riverside County", "San Bernardino County", "Los Angeles County"))
...
### Exploring the Data with Initial Visualization
...
ie_kids <- ie %>% 
  ...
  mutate(kid_inc = (UNDER5*LOWINCPCT))
...
ie_lead <- ie %>% 
  ...
  mutate(kid_lead = (UNDER5*PRE1960PCT))
